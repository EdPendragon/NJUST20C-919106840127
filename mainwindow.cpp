    ...
    ui(new Ui::MainWindow)
{
    ...
    QPushButton* pveb=new QPushButton;      //创建初始化pve模式的按钮
    pveb->setParent(this);
    pveb->setText("人机对战模式");
    pveb->move(450,12);
    pveb->resize(150,75);
...
void MainWindow::paintEvent(QPaintEvent *event){              //绘制棋盘函数的实现
    ...
    for(int i=2; i<msize-1; i++){                     //一共画23-2条竖线和横线
        painter.drawLine(d_side+l_block*i,d_side+l_block*2,
                         d_side+l_block*i,size().height()-d_side-l_block*2);       //第i+1条竖线
        painter.drawLine(d_side+l_block*2,d_side+l_block*i,
                         size().width()-d_side-l_block*2,d_side+l_block*i);      //第i+1条横线
    }
    //绘制鼠标选中的点
    QBrush brush;       //绘图刷子
    ...
    if(clickhang>0&&clicklie>0&&clickhang<msize&&clicklie<msize&&
        ...
        if(btnValue==QMessageBox::Ok){                              //点击对话框
            ...
            game->status=Running;                                   //游戏状态改成正在运行
            if(btnValue==QMessageBox::Ok){                              //点击对话框
                game->gameStart(type);                                  //重新开始游戏
                game->status=Running;                                   //游戏状态改成正在运行
            }
    ...
    if(game->num==400){
        game->status=Win;           //实际上是和棋，不过前边没有设置和棋的状态，所以用Win来代替
        QString shenglifang;
        shenglifang="No one";
        QMessageBox::StandardButton btnValue = QMessageBox::information(this, "The result is ", shenglifang + " has won.");
        if(btnValue==QMessageBox::Ok){                              //点击对话框
...
void MainWindow::mouseMoveEvent(QMouseEvent* event){        //鼠标监听
    ...
    if(x>d_side+l_block*2&&x<size().width()-d_side-l_block*2&&y>d_side+l_block*2&&y<=size().height()-d_side-l_block*2){
...
void MainWindow::mouseReleaseEvent(QMouseEvent *event){
    if(type==pvp||type==pve){
        ...
        peoinput();
        if(type==pve){
            QTimer::singleShot(100, this, SLOT(aiinput()));
        }
    }
    if(type==eve){
            QTimer::singleShot(50, this, SLOT(aiinput()));
            QTimer::singleShot(650, this, SLOT(aiinput()));
    }
}
